...
MommyBot can read completed nightly analysis documents using a dedicated admin-issued `reports:read` token. Enforce `source='daily'` and a saved schedule date on both feed and document reads; manual runs must never leak through direct IDs or optional filters. Keep credentials separate from wallet grants and recheck the issuing admin's live role. Commit `ai_report_feed` cursors with completed documents, using completion order rather than job creation order. See [AI_REPORT_API.md](AI_REPORT_API.md).
