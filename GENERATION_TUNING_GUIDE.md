...
Observation and roll records are independent. `kind: observation` stores intake since the preceding saved check-in with `liquidsMode: interval`, position and diaper counters; it has no probability or outcome. `kind: roll` stores the real draw time, computed probability and result, and takes no inputs from the observation form. Standalone roll metadata is not editable; individual draws can be deleted without clearing an active cooldown. Legacy combined observations remain editable with their original cumulative intake semantics.

Daily liquid totals add intervals by the check-in's recorded local date, including intervals spanning midnight. For mixed legacy/new days, use the maximum legacy cumulative amount plus interval amounts strictly after the latest legacy snapshot; earlier intervals are assumed already covered. The history and edit form distinguish interval amounts from cumulative amounts. Both CSV exports expose measurement mode (`liquidsMode` in device CSV, `liquids_mode` in administrator CSV); rolls and wettings have no intake. Existing SQLite version 2 JSON payload storage supports these record kinds without another table migration.
