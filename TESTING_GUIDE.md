...
Community support: run node --test tests/community-support.test.mjs for default preferences, one-time existing-subscriber enrollment across both schema versions, persisted opt-outs after restart, daily record and roll triggers, recipient snapshots, duplicate prevention, quiet hours, opt-outs, disabled accounts, restart recovery, expired endpoints and anonymous push rendering. The notifications browser fixture also verifies default-on, saving an opt-out, reload/new-device persistence and responsive layout. Push transport is mocked; production delivery requires configured Web Push and an actual subscribed device.
