...
The first new observation, wetting, diaper change or roll received by the server each day creates the same daily check-in used by login bonuses, using the account's pinned timezone. Participating subscribers broadcast an anonymous check-in notice to other participating subscribers; no names, record types, amounts, streaks or record details are included. Additional records or rolls that day, edits and sync retries do not broadcast. Offline/backdated records qualify on their sync day; old check-ins are never backfilled.
