...
For each deployment changing frontend assets, increment the cache version in sw.js and deploy the complete matching set of public files. The current manual cache is v21-mobile-record-change. Keep sw.js revalidated. New workers wait for existing app tabs to close, preventing mixed assets during a check-in.
