...
export function analyzeDataset(dataset, {from='',to='',interval='day',timeBinHours=6}={}) { // Compute descriptive statistics per participant before aggregating; cumulative snapshots never become extra events.
  ...
  for(const user of dataset.users) {
    ...
    for(const entry of entries) {
      ...
      if(entry.kind==='wetting') {
        ...
        daily.sum+=score;daily.n++;daily.participants.add(user.id);actionDays.set(day,daily); // One actual classified wetting contributes one score; snapshots and random rolls do not.
  ...
  graph('position-action','Action by position','Counts of actual classified wettings, split by position and action. Exact data includes the total for each position.','bar',['Position',...actionLabels,'Total'],[...positionActions].map(([position,counts])=>[positionLabels[position],...counts,counts.reduce((a,b)=>a+b,0)]));
  graphs.at(-1).plotColumns=[1,2,3,4,5];
  const hourLabel=hour=>String(hour).padStart(2,'0')+':00';
  graph('time-action','Action by time of day','Counts in '+timeBinHours+'-hour groups using recorded local time for each event. Start inclusive, end exclusive; unlogged events are unknown.','bar',['Local time',...actionLabels,'Total'],timeActions.map((counts,i)=>[hourLabel(i*timeBinHours)+' to '+hourLabel((i+1)*timeBinHours),...counts,counts.reduce((a,b)=>a+b,0)]));
  graphs.at(-1).plotColumns=[1,2,3,4,5];
  graph('action-score','Daily action score','F=1, SF=2, V=3, SI=4, I=5. Daily event-weighted mean for the selected participants; choose one participant for an individual trend. Ordinal category spacing is assumed, not measured. This describes recorded actions, not proof of training effectiveness. Always grouped by day; missing days have gaps.','line',['Day','Mean action score','Classified wettings','Participants'],[...actionDays].sort((a,b)=>a[0].localeCompare(b[0])).map(([day,value])=>[day,value.sum/value.n,value.n,value.participants.size]));
