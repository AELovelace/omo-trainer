...
export const CHART_METRICS=[
 ...
 ['bedwetting','Bedwetting','count'],['used-the-potty','Used the potty','count'], // Keep the new event categories available as independent chart metrics.
...
function aggregate(users,filters) { // Reuse the audited intake rules, then add period counts and sample-weighted means from actual events.
 ...
 const get=label=>{if(!rows.has(label))rows.set(label,{period:label,...Object.fromEntries(CHART_METRICS.map(m=>[m.id,['probability','action','interval','meanChangedWettings','diaperNumber','liquids'].includes(m.id)?null:0])),actionSum:0,actionN:0,intervalSum:0,intervalN:0});return rows.get(label);};
 ...
 for(const {entry} of view.records){const row=get(period(entry.occurredAt.slice(0,10),filters.interval));
  if(entry.kind==='wetting'){row[entry.category]++;row[entry.position]++;const score=ACTION_SCORES[entry.category];if(score!==undefined){row.actionSum+=score;row.actionN++;}} // Average only events on the existing five-category score scale.
 ...
 return [...rows.values()].sort((a,b)=>a.period.localeCompare(b.period)).map(row=>({...row,liquids:row.observations?row.liquids:null,action:row.actionN?row.actionSum/row.actionN:null,interval:row.intervalN?row.intervalSum/row.intervalN:null}));
}
