...
function payload(sub) {return {subscription:sub.toJSON(),timeZone:timezone(),quietStart:Number($('#notification-quiet-start').value),quietEnd:Number($('#notification-quiet-end').value),adminMessages:$('#notification-admin-messages').checked};}
...
$('#notification-save').addEventListener('click',async()=>{
 ...
 try{const sub=await registration.pushManager.getSubscription();if(!sub)throw Error('Enable notifications on this device first.');config={...config,...await request('notifications',payload(sub))};status('Timezone and quiet hours saved.');}
