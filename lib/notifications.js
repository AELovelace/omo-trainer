...
function controls() {
 for(const id of ['social-likes','social-comments','friend-posts','friend-wettings','friend-changes','friend-liquids'])$('#notification-'+id).disabled=busy||!config;
...
function payload(sub) {return {subscription:sub.toJSON(),timeZone:timezone(),quietStart:Number($('#notification-quiet-start').value),quietEnd:Number($('#notification-quiet-end').value),adminMessages:$('#notification-admin-messages').checked,communitySupport:$('#notification-community-support').checked,communityAnonymous:$('#notification-community-anonymous').checked,communityFriendsOnly:$('#notification-community-friends-only').checked,socialLikes:$('#notification-social-likes').checked,socialComments:$('#notification-social-comments').checked,friendPosts:$('#notification-friend-posts').checked,friendWettings:$('#notification-friend-wettings').checked,friendChanges:$('#notification-friend-changes').checked,friendLiquids:$('#notification-friend-liquids').checked};}
async function refresh() {
 if(busy)return;busy=true;controls();$('#notification-timezone').textContent=timezone();
 try {
  config=await request('notifications');
  for(const [id,field] of [['social-likes','socialLikes'],['social-comments','socialComments'],['friend-posts','friendPosts'],['friend-wettings','friendWettings'],['friend-changes','friendChanges'],['friend-liquids','friendLiquids']])$('#notification-'+id).checked=config.preferences?.[field]===undefined?true:Boolean(config.preferences[field]);
