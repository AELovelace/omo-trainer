...
function payload(sub) {return {subscription:sub.toJSON(),timeZone:timezone(),quietStart:Number($('#notification-quiet-start').value),quietEnd:Number($('#notification-quiet-end').value)};}
async function refresh() {
 if(busy)return;busy=true;controls();$('#notification-timezone').textContent=timezone();
 try {
  config=await request('notifications');
  if(config.preferences){$('#notification-quiet-start').value=config.preferences.quietStart;$('#notification-quiet-end').value=config.preferences.quietEnd;}
  ...
  status(!config.configured?'Notifications are waiting for server setup.':Notification.permission==='denied'?'Notifications are blocked. You can allow them in your browser settings.':sub&&config.subscriptions?'Reminders are enabled on this device.':'Reminders are off on this device.');
...
$('#notification-enable').addEventListener('click',async()=>{
 ...
 try {
  ...
  const value=await request('notifications',payload(sub));config={...config,...value};status('Reminders enabled. A little potty check-in may arrive near your estimated time.');
...
$('#notification-save').addEventListener('click',async()=>{
 ...
 try{const sub=await registration.pushManager.getSubscription();if(!sub)throw Error('Enable reminders on this device first.');config={...config,...await request('notifications',payload(sub))};status('Timezone and quiet hours saved.');}
 catch(error){status(error.message);}finally{busy=false;controls();}
});
$('#notification-disable').addEventListener('click',async()=>{
 if(busy)return;busy=true;controls();
 try{config={...config,...await request('notifications/disable',{all:true})};const sub=await registration?.pushManager.getSubscription();await sub?.unsubscribe();status('Reminders turned off on all your devices.');}
 catch(error){status(error.message);}finally{busy=false;controls();}
});
