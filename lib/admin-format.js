...
export const ENTRY_FIELDS = ['id','kind','occurredAt','liquidsMl','liquidsMode','position','diaperNumber','wettingsCount','probability','result','source','edited','category','rolledAt','rolledResult','protocolVersion','timeZone','lastFailureAt','desperation','baseProbability','probabilityModifier','rollRuleVersion','desperationMode','lastFailureDesperationMode'];
...
export function csvEntry(row) { // Accept the web export and the operator's snake_case export while preserving legacy cumulative semantics.
  const aliases={ entry_id:'id', occurred_at:'occurredAt', liquids_ml:'liquidsMl', diaper_number:'diaperNumber', wettings_count:'wettingsCount',
    rolled_at:'rolledAt', rolled_result:'rolledResult', protocol_version:'protocolVersion', time_zone:'timeZone', last_failure_at:'lastFailureAt', liquids_mode:'liquidsMode', base_probability:'baseProbability', probability_modifier:'probabilityModifier', roll_rule_version:'rollRuleVersion', desperation_mode:'desperationMode', last_failure_desperation_mode:'lastFailureDesperationMode' };
  const input={...row};
  ...
  for(const key of ENTRY_FIELDS) if(input[key]!==undefined && input[key]!=='' && input[key]!==null) {
    ...
    else if(['edited','desperationMode','lastFailureDesperationMode'].includes(key)) {
      if(!['true','false','1','0',true,false,0,1].includes(input[key])) throw Error('Invalid CSV boolean flag.');
