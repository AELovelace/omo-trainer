...
export const ENTRY_FIELDS = ['id','kind','occurredAt','liquidsMl','liquidsMode','position','diaperNumber','wettingsCount','probability','result','source','edited','category','rolledAt','rolledResult','protocolVersion','timeZone','lastFailureAt','desperation'];
...
const numeric = new Set(['liquidsMl','diaperNumber','wettingsCount','probability','protocolVersion']);
...
export function csvEntry(row) { // Accept the web export and the operator's snake_case export while preserving legacy cumulative semantics.
  const aliases={ entry_id:'id', occurred_at:'occurredAt', liquids_ml:'liquidsMl', diaper_number:'diaperNumber', wettings_count:'wettingsCount',
    rolled_at:'rolledAt', rolled_result:'rolledResult', protocol_version:'protocolVersion', time_zone:'timeZone', last_failure_at:'lastFailureAt', liquids_mode:'liquidsMode' };
  const input={...row};
