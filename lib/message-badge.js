...
async function refresh(){
 ...
 try{const response=await fetch('./api/social/messages/unread',{credentials:'same-origin',cache:'no-store',signal:AbortSignal.timeout(15000)});if(ticket!==epoch)return;if(!response.ok){show(0);return;}const value=await response.json();if(ticket===epoch)show(Number.isSafeInteger(value.unread)&&value.unread>=0?value.unread:0);}
...
window.addEventListener('little-log-signout',clear);window.addEventListener('offline',clear);window.addEventListener('storage',event=>{if((event.key==='lidoll.little-log.v1'||event.key===null)&&!event.newValue)clear();});
