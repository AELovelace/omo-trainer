...
function buttons(){
 ...
 $('#status-post').disabled=posting||preparing||!account||!navigator.onLine;$('#status-pictures').disabled=posting||preparing;$('#status-text').disabled=posting;$('#status-audience').disabled=posting;
 $('#message-send').disabled=sending||!peer||!account||!navigator.onLine;$('#message-text').disabled=sending;$('#message-friend').disabled=sending||chatBusy;
 $('#feed-refresh').disabled=feedBusy;$('#message-refresh').disabled=chatBusy;$('#feed-older').hidden=feedNext===null;$('#feed-newest').hidden=feedBefore===null;$('#messages-older').hidden=messageNext===null;$('#messages-newest').hidden=messageBefore===null;
}
function clearViews(){epoch++;$('#activity-list').replaceChildren();$('#activity-status').textContent='';$('#activity-read-all').disabled=true;activityLatest=null;$('#status-feed').replaceChildren();$('#conversation-list').replaceChildren();$('#message-list').replaceChildren();$('#message-friend').replaceChildren();feedNext=null;messageNext=null;buttons();} // Private views are never retained in an offline cache.
function reset(){clearViews();account=null;pictures=[];peer='';activityBefore=null;activityNext=null;postRetry=null;messageRetry=null;feedBefore=null;messageBefore=null;$('#status-text').value='';$('#status-audience').value='friends';$('#status-pictures').value='';$('#status-picture-preview').replaceChildren();$('#message-text').value='';buttons();}
async function request(path,input){
 ...
 let value;try{value=await response.json();}catch{throw Error(response.status===413?'These pictures are too large for the server. Try fewer or smaller pictures.':'The server could not complete this request. Please retry.');}
 if(!response.ok){if(response.status===401||response.status===403)reset();throw Error(value.error||'Could not load this page.');}
...
async function preparePicture(file){
 if(!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>20*1024*1024)throw Error('Choose JPEG, PNG or WebP pictures up to 20 MB each.');
 const bitmap=await createImageBitmap(file);
 try {
  const scale=Math.min(1,1600/bitmap.width,1600/bitmap.height),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));
  ...
  let data=canvas.toDataURL('image/jpeg',0.82).split(',')[1];if(data.length>2*1024*1024)data=canvas.toDataURL('image/jpeg',0.6).split(',')[1];if(data.length>2*1024*1024)throw Error('This picture is too large. Choose a smaller picture.');return {data,alt:''};
 }finally{bitmap.close();}
} // Resize before uploading; the server independently verifies and re-encodes the submitted bytes.
$('#status-pictures').addEventListener('change',async()=>{
 ...
 preparing=true;buttons();const ticket=epoch;
...
$('#status-form').addEventListener('submit',async event=>{
 ...
 try{postRetry??={requestId:crypto.randomUUID(),body:$('#status-text').value,audience:$('#status-audience').value,pictures:pictures.map(p=>({...p}))};const result=await request('posts',postRetry);if(ticket!==epoch)return;
  $('#status-compose-status').textContent=result.deleted?'This post was already deleted.':'Status posted to '+(postRetry.audience==='public'?'Public':'Friends')+'.';$('#status-text').value='';pictures=[];postRetry=null;renderPictures();feedBefore=null;
 }catch(error){$('#status-compose-status').textContent=error.message;return;}finally{posting=false;buttons();}await loadFeed();
});
async function loadFeed(){
 ...
 try{const postId=new URLSearchParams(location.hash.split('?')[1]??'').get('post');const value=await request(postId?'post?id='+encodeURIComponent(postId):'feed?audience='+$('#feed-audience').value+(feedBefore?'&before='+feedBefore:''));if(ticket!==epoch)return;
  const container=$('#status-feed');container.replaceChildren();feedNext=value.nextBefore;
  for(const post of value.items){const card=el('article',undefined,'status-card'),header=el('div',undefined,'card-heading');header.append(el('h3',post.author.label),el('span',post.audience==='public'?'Public':'Friends','pill'));card.append(header,el('p',stamp(post.created),'field-help'),el('p',post.body,'social-body'));
   if(post.pictures.length){const gallery=el('div',undefined,'status-gallery');for(const picture of post.pictures){const img=el('img');img.src='./api/social/picture?id='+encodeURIComponent(picture.id);img.alt=picture.alt||'Picture shared by '+post.author.label;img.loading='lazy';img.width=picture.width;img.height=picture.height;gallery.append(img);}card.append(gallery);}
   card.append(postActions(post));if(postId)card.append(button('Back to feed',()=>{location.hash='#feed';}));
   if(post.author.id===account.participant.id)card.append(button('Delete post',async()=>{if(!confirm('Delete this status and its pictures?'))return;try{await request('posts/delete',{id:post.id});await loadFeed();}catch(error){$('#feed-status').textContent=error.message;}}));container.append(card);
  }
  $('#feed-status').textContent=value.items.length?'Updates are visible to the audience shown on each post.':'No updates yet. Share a little of your day.';
 }catch(error){$('#status-feed').replaceChildren();$('#feed-status').textContent=error.message;}finally{feedBusy=false;buttons();}
}
async function loadChat(){
 if(chatBusy||sending||!navigator.onLine)return;chatBusy=true;buttons();const ticket=epoch;
 try{const data=await request('conversations');if(ticket!==epoch)return;
  const select=$('#message-friend');select.replaceChildren(new Option('Choose a friend',''));$('#conversation-list').replaceChildren();
  for(const convo of data.conversations){select.append(new Option(convo.friend.label+(convo.unread?' ('+convo.unread+' unread)':''),convo.friend.id));
   const row=button(convo.friend.label+(convo.unread?' · '+convo.unread+' unread':''),()=>choosePeer(convo.friend.id));row.classList.add('conversation-button');row.dataset.peer=convo.friend.id;$('#conversation-list').append(row);
  }
  if(!data.conversations.some(c=>c.friend.id===peer)){peer='';$('#message-text').value='';messageRetry=null;}select.value=peer;
  ...
  for(const message of value.items){const own=message.sender===account.participant.id,card=el('article',undefined,'message-bubble'+(own?' message-own':''));card.append(el('strong',own?'You':currentConversation.friend.label),el('p',message.deleted?'Message removed':message.body,'social-body'),el('small',stamp(message.created)));
  ...
  $('#message-status').textContent=value.items.length?'Private between you and this friend. Administrators can review messages that either of you reports.':'Say hello to your friend.';
  if(value.items.length&&!document.hidden){
   await request('messages/read',{participantId:peer,seq:value.items.at(-1).seq});if(ticket!==epoch)return;
   if(value.items.at(-1).seq>=(currentConversation.latest?.seq??0)){ // Clear the visible unread labels only when this page reaches the latest known message.
    ...
    for(const row of $('#conversation-list').children)if(row.dataset.peer===peer)row.textContent=currentConversation.friend.label;
   }
  }
 }catch(error){$('#message-list').replaceChildren();$('#message-status').textContent=error.message;}finally{chatBusy=false;buttons();}
} // Poll only the visible conversation; CSRF-protected read markers never advance beyond messages actually loaded.
function choosePeer(id){if(sending||chatBusy)return;if(peer!==id){peer=id;messageBefore=null;messageRetry=null;$('#message-text').value='';$('#message-list').replaceChildren();}void loadChat();}
...
$('#message-form').addEventListener('submit',async event=>{
 ...
 try{messageRetry??={requestId:crypto.randomUUID(),participantId:peer,body:$('#message-text').value};await request('messages',messageRetry);if(ticket!==epoch)return;messageRetry=null;$('#message-text').value='';messageBefore=null;}
...
function visible(){if(route()==='#feed')void loadFeed();if(route()==='#messages')void loadChat();if(route()==='#activity')void loadActivity();}
window.addEventListener('hashchange',()=>{clearViews();visible();});window.addEventListener('online',visible);
...
window.addEventListener('little-log-signout',reset);window.addEventListener('storage',e=>{if((e.key==='lidoll.little-log.v1'||e.key===null)&&!e.newValue)reset();});
document.addEventListener('visibilitychange',()=>{if(document.hidden)clearViews();else visible();});
document.addEventListener('little-log-message-friend',e=>{peer=e.detail;messageBefore=null;messageRetry=null;$('#message-text').value='';if(location.hash==='#messages')void loadChat();else location.hash='#messages';});
...
function postActions(post) {
 const box=el('div'),actions=el('div',undefined,'social-actions'),status=el('p','','field-help');status.setAttribute('role','status');
 const like=button((post.liked?'Unlike':'Like')+' · '+post.likes,async()=>{like.disabled=true;try{const result=await request('like',{postId:post.id,liked:!post.liked});post.liked=result.liked;post.likes=result.likes;like.textContent=(post.liked?'Unlike':'Like')+' · '+post.likes;like.setAttribute('aria-pressed',String(post.liked));}catch(error){status.textContent=error.message;}finally{like.disabled=false;}});like.setAttribute('aria-pressed',String(post.liked));actions.append(like,button('Report post',()=>reportContent('post',post.id,status)));
 const details=el('details',undefined,'social-comments'),summary=el('summary','Comments · '+post.comments),list=el('div'),form=el('form'),label=el('label','Write a comment'),input=el('textarea'),submit=el('button','Post comment','button primary'),older=button('Older comments',()=>void load(next));
 input.maxLength=2000;input.required=true;input.rows=3;label.append(input);submit.type='submit';form.append(label,submit);older.hidden=true;details.append(summary,list,older,form);box.append(actions,status,details);
 let busy=false,next=null,retry=null;const ticket=epoch;
 input.addEventListener('input',()=>{retry=null;});
 async function load(before=null){if(busy)return;busy=true;submit.disabled=true;older.disabled=true;
  try{const value=await request('comments?postId='+encodeURIComponent(post.id)+(before?'&before='+before:''));if(ticket!==epoch)return;list.replaceChildren();next=value.nextBefore;older.hidden=next===null;
   for(const comment of value.items){const card=el('article',undefined,'social-comment');card.append(el('strong',comment.author.label),el('p',comment.body,'social-body'),el('small',stamp(comment.created)));
    const controls=el('div',undefined,'social-actions');controls.append(button('Report comment',()=>reportContent('comment',comment.id,status)));
    if(comment.author.id===account.participant.id||post.author.id===account.participant.id)controls.append(button('Delete comment',async()=>{if(!confirm('Delete this comment?'))return;try{await request('comments/delete',{id:comment.id});post.comments=Math.max(0,post.comments-1);summary.textContent='Comments · '+post.comments;await load();}catch(error){status.textContent=error.message;}}));card.append(controls);list.append(card);
   }
   if(!value.items.length)list.append(el('p','No comments yet.','field-help'));
  }catch(error){status.textContent=error.message;list.replaceChildren();}finally{busy=false;submit.disabled=false;older.disabled=false;}
 }
 ...
 form.addEventListener('submit',async event=>{event.preventDefault();if(busy)return;busy=true;submit.disabled=true;input.disabled=true;
  try{retry??={requestId:crypto.randomUUID(),postId:post.id,body:input.value};const result=await request('comments',retry);if(ticket!==epoch)return;input.value='';retry=null;if(!result.repeated)post.comments++;summary.textContent='Comments · '+post.comments;status.textContent='Comment posted.';}
  catch(error){status.textContent=error.message;return;}finally{busy=false;submit.disabled=false;input.disabled=false;}await load();
 });return box;
