...
function clearViews(){epoch++;memberBusy=false;$('#member-details').replaceChildren();clearPostGalleries($('#member-posts'));$('#member-status').textContent='';memberNext=null;memberControls();$('#activity-list').replaceChildren();$('#activity-status').textContent='';$('#activity-read-all').disabled=true;activityLatest=null;clearPostGalleries($('#status-feed'));$('#conversation-list').replaceChildren();$('#message-list').replaceChildren();$('#message-friend').replaceChildren();feedNext=null;messageNext=null;buttons();} // Private views are never retained in an offline cache.
function reset(){memberBefore=null;clearViews();$('#status-view-post').hidden=true;$('#status-view-post').href='#feed';$('#post-status').textContent='Sign in to post an update.';account=null;pictures=[];peer='';activityBefore=null;activityNext=null;postRetry=null;messageRetry=null;feedBefore=null;messageBefore=null;$('#status-text').value='';$('#status-audience').value='friends';$('#status-pictures').value='';$('#status-picture-preview').replaceChildren();$('#message-text').value='';buttons();}
...
function renderPosts(posts,container,{detail=false,refresh=loadFeed,status=$('#feed-status')}={}){
  for(const post of posts){const card=el('article',undefined,'status-card'),header=el('div',undefined,'card-heading');header.append(createIdentity(post.author,'h3'),el('span',post.audience==='public'?'Public':'Friends','pill'));card.append(header,el('p',stamp(post.created),'field-help'),el('p',post.body,'social-body'));
   if(post.pictures.length)card.append(createPostGallery(post));
   card.append(postActions(post));if(detail)card.append(button('Back to feed',()=>{location.hash='#feed';}));
   if(post.author.id===account.participant.id)card.append(button('Delete post',async()=>{if(!confirm('Delete this status and its pictures?'))return;try{await request('posts/delete',{id:post.id});await refresh();}catch(error){status.textContent=error.message;}}));container.append(card);
  }
}
...
async function loadChat(){
 ...
 try{const data=await request('conversations');if(ticket!==epoch)return;
  ...
  for(const convo of data.conversations){select.append(new Option(convo.friend.label+(convo.unread?' ('+convo.unread+' unread)':''),convo.friend.id));
   const row=el('div',undefined,'conversation-row'),open=button('Message'+(convo.unread?' · '+convo.unread+' unread':''),()=>choosePeer(convo.friend.id));open.classList.add('conversation-button');row.append(createIdentity(convo.friend),open);row.dataset.peer=convo.friend.id;$('#conversation-list').append(row);
  }
  ...
  if(value.items.length&&!document.hidden){
   ...
   if(value.items.at(-1).seq>=(currentConversation.latest?.seq??0)){ // Clear the visible unread labels only when this page reaches the latest known message.
    ...
    for(const row of $('#conversation-list').children)if(row.dataset.peer===peer)row.querySelector('button').textContent='Message';
   }
  }
