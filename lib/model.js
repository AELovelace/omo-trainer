...
export function validateEntry(entry) { // Whitelists persisted fields so imported markup or extra properties never enter the interface.
  ...
  if (entry.kind === 'roll') {
    ...
    if(hasVariation) { // Versioned metadata distinguishes the daily base from the actual one-roll chance in every export and sync.
      if(![1,2].includes(entry.rollRuleVersion))throw new Error('Unsupported roll probability rule.');
    ...
    return { id: entry.id, kind: 'roll', occurredAt: validateTimestamp(entry.occurredAt), probability: integer(entry.probability, hasVariation?10:20, hasVariation?100:80, 'Probability'), ...variation,
      result: entry.result, source: 'random', rolledAt: entry.occurredAt, rolledResult: entry.result,
      ...(entry.position === undefined ? {} : { position: entry.position }),
      ...(entry.desperation === undefined ? {} : { desperation: entry.desperation }) }; // Older rolls retain an unrecorded level instead of being assigned Low.
  }
...
export function rollResult(probability, random = () => crypto.getRandomValues(new Uint32Array(1))[0]) { // Uses rejection sampling to give all 100 integer outcomes exactly equal weight.
  ...
  return uniformDraw(100,random) < probability ? 'pee' : 'hold';
}
...
export function rollProbability(baseProbability,random=()=>crypto.getRandomValues(new Uint32Array(1))[0]) { // Version 2: 2.5% each guaranteed/down/up, 92.5% unchanged; +/- events persist through protocol replay.
  ...
  return {baseProbability,probability:adjustedRollProbability(baseProbability,probabilityModifier,2),probabilityModifier,rollRuleVersion:2};
}
...
export function toCsv(entries) { // Exports all selected snapshots with explicit units, offsets, provenance, and escaped CSV cells.
  const columns = ['occurredAt', 'liquidsMl', 'position', 'diaperNumber', 'wettingsCount', 'probability', 'result', 'source', 'edited', 'kind', 'category', 'rolledAt', 'rolledResult', 'protocolVersion', 'timeZone', 'lastFailureAt', 'liquidsMode', 'desperation', 'baseProbability', 'probabilityModifier', 'rollRuleVersion'];
