...
export function daySummary(entries, day) { // Treats liquids as a cumulative snapshot, never a sum of repeated snapshots.
  const matches = sortedEntries(entries.filter(entry => entry.occurredAt.slice(0, 10) === day));
  return {
...
export function toCsv(entries) { // Exports all selected snapshots with explicit units, offsets, provenance, and escaped CSV cells.
  const columns = ['occurredAt', 'liquidsMl', 'position', 'diaperNumber', 'wettingsCount', 'probability', 'result', 'source', 'edited'];
  const escape = value => `"${String(value).replaceAll('"', '""')}"`;
  return '\uFEFF' + [columns, ...sortedEntries(entries).map(entry => columns.map(key => entry[key]))]
