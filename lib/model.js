...
export const WETTING_CATEGORIES = ['forced', 'voluntary', 'semi-involuntary', 'involuntary'];
export const isRoll = entry => entry.kind === undefined; // Legacy snapshots retain their exact schema and retry fingerprints.
...
export function daySummary(entries, day) { // Treats liquids as a cumulative snapshot, never a sum of repeated snapshots.
  const matches = sortedEntries(entries.filter(entry => isRoll(entry) && entry.occurredAt.slice(0, 10) === day));
  return {
    count: matches.length,
    pee: matches.filter(entry => entry.result === 'pee').length,
    hold: matches.filter(entry => entry.result === 'hold').length,
    liquidsMl: matches.reduce((maximum, entry) => Math.max(maximum, entry.liquidsMl), 0),
...
export function dailySeries(entries, length, today = new Date()) { // Walks calendar days at noon so DST changes cannot duplicate or skip dates.
  ...
  for (const entry of entries) {
    if (!isRoll(entry)) continue;
    const day = entry.occurredAt.slice(0, 10);
    const totals = grouped.get(day) ?? { pee: 0, hold: 0, liquidsMl: 0 };
    totals[entry.result] += 1;
    totals.liquidsMl = Math.max(totals.liquidsMl, entry.liquidsMl);
    grouped.set(day, totals);
  }
  return Array.from({ length }, (_, index) => {
    const date = new Date(today.getFullYear(), today.getMonth(), today.getDate() - length + index + 1, 12);
    const day = localDay(date);
    return { day, ...(grouped.get(day) ?? { pee: 0, hold: 0, liquidsMl: 0 }) };
  });
}
...
export function toCsv(entries) { // Exports all selected snapshots with explicit units, offsets, provenance, and escaped CSV cells.
  const columns = ['occurredAt', 'liquidsMl', 'position', 'diaperNumber', 'wettingsCount', 'probability', 'result', 'source', 'edited', 'kind', 'category', 'rolledAt', 'rolledResult', 'protocolVersion', 'timeZone', 'lastFailureAt'];
  const escape = value => `"${String(value).replaceAll('"', '""')}"`;
  return '\uFEFF' + [columns, ...sortedEntries(entries).map(entry => columns.map(key => key === 'kind' ? entry.kind ?? 'roll' : entry[key] ?? ''))]
