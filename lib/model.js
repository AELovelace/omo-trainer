...
export function validateEntry(entry) { // Whitelists persisted fields so imported markup or extra properties never enter the interface.
  ...
  if (entry.kind === 'observation') {
    if (!POSITIONS.includes(entry.position) || entry.liquidsMode !== 'interval') throw new Error('Invalid observation position or liquid measurement mode.');
    ...
    return { id: entry.id, kind: 'observation', occurredAt: validateTimestamp(entry.occurredAt),
      liquidsMl: integer(entry.liquidsMl, 0, 1000000, 'Liquids'), liquidsMode: 'interval', position: entry.position,
      diaperNumber: integer(entry.diaperNumber, 1, 10000, 'Diaper number'), wettingsCount: integer(entry.wettingsCount, 0, 10000, 'Wettings'), edited: entry.edited ?? false };
  }
  if (entry.kind === 'roll') {
