...
export function createIdentity(person,tag='strong',label=person.label) {
 const identity=document.createElement('span');identity.className='social-identity';const name=document.createElement(tag);name.textContent=label;identity.append(createAvatar(person),name);return identity;
} // Keep member names as plain text beside their avatar in each social view.
