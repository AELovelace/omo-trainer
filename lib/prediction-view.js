...
export function clearPrediction() { // Administrator views must release model inputs and rendered values immediately when access or participant selection changes.
  cachedEntries=undefined;cachedAccount=undefined;cachedDay=undefined;model=undefined;lastMinute=-1;
  for(const name of ['evidence','status','interval','intake','model','detail','validation','window','likely','scale'])$('prediction-'+name).textContent='';
  for(const name of ['bars','data'])$('prediction-'+name).replaceChildren();
  for(const name of ['result','plot','table-wrap'])$('prediction-'+name).hidden=true;
}
