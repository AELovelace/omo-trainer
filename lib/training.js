...
export const COOLDOWN_MS = 15 * 60 * 1000; // A random Hold blocks another roll for fifteen minutes from its original timestamp.
...
export function trainingState(entries, now = new Date()) { // Replays completed calendar days once; rerenders and restarts cannot apply a day twice.
  ...
  for(const entry of entries.filter(e=>e.kind==='roll'&&[2,3].includes(e.rollRuleVersion)&&Date.parse(e.occurredAt)<=Number(now)).sort((a,b)=>Date.parse(a.occurredAt)-Date.parse(b.occurredAt)||a.id.localeCompare(b.id))) {
...
export function cooldownRemaining(entries, now = Date.now()) { // Original draw timestamps/results survive corrections; manual observations never start a cooldown.
  let until = 0;
  for (const entry of entries) {
    if (entry.kind === 'protocol' && entry.lastFailureAt) until = Math.max(until, Date.parse(entry.lastFailureAt) + COOLDOWN_MS*(entry.lastFailureDesperationMode===true?2:1)); // Deleting a desperation roll cannot shorten its saved deadline.
    if (isRoll(entry) && entry.source === 'random' && (entry.rolledResult ?? entry.result) === 'hold') {
      until = Math.max(until, Date.parse(entry.rolledAt ?? entry.occurredAt) + COOLDOWN_MS*(entry.desperationMode===true?2:1));
    }
  }
