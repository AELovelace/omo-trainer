...
async function request(path='',input) {
 ...
 const value=await response.json();if(!response.ok){if(response.status===401||response.status===403)clear($('#friend-share-dialog').open);throw Error(value.error||'Friends could not be loaded.');}
...
function renderFriends(items) {
 ...
 for(const friend of items){
  const item=node('article',undefined,'friend-row'),description=node('div');description.append(node('strong',friend.label),node('p',friend.state==='accepted'?'Friends':friend.direction==='incoming'?'Wants to be your friend':'Request sent','field-help'));
  ...
  item.append(actionButton(label,()=>{if(friend.state!=='accepted'||confirm('Remove this friend? Shared-record access between you will end.'))void mutate('',{action:'remove',id:friend.id});}));list.append(item);
 }
...
async function loadShared(ticket=generation) {
 ...
 for(const item of data.items){const card=node('article',undefined,'friend-shared-card'),outgoing=$('#friend-shared-direction').value==='outgoing';
  card.append(node('h3',(outgoing?'Shared with ':'From ')+item.friend.label),details(item.record));
...
$('#friend-search-form').addEventListener('submit',async event=>{
 ...
 try {
  ...
  for(const member of data.members){const item=node('div',undefined,'friend-row');item.append(node('strong',member.label));
