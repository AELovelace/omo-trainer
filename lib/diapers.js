...
export function diaperAtTime(entries, occurredAt) { // Assign a wetting using records known at its event time; later changes cannot shift a backdated event.
  return diaperSummary(entries.filter(entry=>Date.parse(entry.occurredAt)<=Date.parse(occurredAt)),occurredAt.slice(0,10)).currentDiaper;
}
