...
function currentDiaper(entries) { // The latest actual change starts a new diaper; a calendar boundary does not.
  ...
  let next=1;
  if(index>=0) {
    const change=ordered[index],day=change.occurredAt.slice(0,10),earlier=ordered.slice(index+1);
    const firstToday=!earlier.some(entry=>entry.kind==='diaper-change'&&entry.occurredAt.slice(0,10)===day);
    const carried=earlier.some(entry=>entry.diaperNumber!==undefined&&entry.occurredAt.slice(0,10)<day);
    next=firstToday&&carried?1:change.diaperNumber+1; // Reset only when changing after a date rollover; midnight itself leaves the worn diaper intact.
  }
  return Math.min(10000,active.reduce((number,entry)=>Math.max(number,entry.diaperNumber??0),next));
}
...
export function suggestedDiaperWettings(entries, day, diaperNumber, before) { // Count the active diaper's whole wear period, including overnight wettings.
  ...
  const relevant=period.filter(entry=>entry.category!=='used-the-potty'&&(entry.diaperNumber===diaperNumber||(active&&entry.kind==='wetting'))); // Toilet visits never add diaper wettings; preserve overnight records from earlier versions.
