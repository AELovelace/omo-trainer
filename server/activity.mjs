...
export function createActivity(db,{canSee=()=>true,now=Date.now}={}) {
 ...
 function record(owner,{source,kind,actor=null,postId=null,commentId=null,title='',body='',created=now()},push=false) {
  ...
  if(push&&pref&&db.prepare(`SELECT 1 FROM notification_preferences WHERE owner=? AND ${pref}=1`).get(owner)) {
 ...
 function cancel(owner) {
  ...
  for(const row of db.prepare("SELECT d.*,n.kind FROM activity_deliveries d JOIN activity_notifications n ON n.id=d.notification_id WHERE d.owner=? AND d.state='queued'").all(owner)) {
   if(!pref?.[preferences[row.kind]]||!db.prepare('SELECT 1 FROM push_subscriptions WHERE owner=? AND endpoint=?').get(owner,row.endpoint))db.prepare("UPDATE activity_deliveries SET state='skipped' WHERE notification_id=? AND endpoint=?").run(row.notification_id,row.endpoint);
  }
 ...
 async function tick(instant,{send,localBlock}) {
  ...
  for(const row of rows) {
   ...
   if(row.created+86400000<=time||!sub||!pref?.[preferences[row.kind]]){db.prepare("UPDATE activity_deliveries SET state='skipped' WHERE notification_id=? AND endpoint=?").run(row.id,row.endpoint);continue;}
