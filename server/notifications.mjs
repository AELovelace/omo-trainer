...
export function createNotifications(db,records,options={}) { // Subscriptions and decisions stay in the scientific database, separate from the market.
 ...
 for(const name of ['social_likes','social_comments','friend_posts','friend_wettings','friend_changes','friend_liquids'])if(!db.prepare('PRAGMA table_info(notification_preferences)').all().some(c=>c.name===name))db.exec('ALTER TABLE notification_preferences ADD COLUMN '+name+' INTEGER NOT NULL DEFAULT 1'); // One-time column defaults enroll existing subscribers; later saves preserve opt-outs.
 ...
 function save(owner,input) { // Authenticated opt-in registers only this user's endpoint and local-time preferences.
  ...
  const savedPreferences=db.prepare('SELECT community_support,community_anonymous,community_friends_only,social_likes,social_comments,friend_posts,friend_wettings,friend_changes,friend_liquids FROM notification_preferences WHERE owner=?').get(owner);
  ...
  const socialValues=[['socialLikes','social_likes'],['socialComments','social_comments'],['friendPosts','friend_posts'],['friendWettings','friend_wettings'],['friendChanges','friend_changes'],['friendLiquids','friend_liquids']].map(([field,column])=>{const value=input[field]===undefined?(savedPreferences?Boolean(savedPreferences[column]):true):input[field];if(typeof value!=='boolean')fail('Choose valid social notification preferences.');return Number(value);});
  ...
  db.exec('BEGIN IMMEDIATE');try {
   ...
   db.prepare('UPDATE notification_preferences SET social_likes=?,social_comments=?,friend_posts=?,friend_wettings=?,friend_changes=?,friend_liquids=? WHERE owner=?').run(...socialValues,owner);options.activity?.cancel(owner);
