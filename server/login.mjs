    ...
    async route(request, response, route) {
      try {
        ...
        if (route === 'callback') {
          ...
          database.deleteSession(cookie(request, sessionName));
          return redirect(response, attempt.returnTo === 'growth-chart' ? `${base}#potty-chart` : attempt.returnTo === 'admin' ? `${base}admin/` : `${base}#settings`, [setCookie(sessionName, database.createSession(account.id), 3600), setCookie(loginName, '', 0)]);
        }
