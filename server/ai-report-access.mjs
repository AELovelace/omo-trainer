...
export function createReportAccess(db,admin,{now=Date.now}={}) { // Report integrations have separate read-only credentials, never ordinary wallet or browser tokens.
  ...
  const metadata='f.cursor,j.id,j.day,j.source,j.created,j.finished,j.model_used,j.finish_reason';
  ...
  function feed(secret,{after,limit}={}) { // Completion cursors prevent an older queued job from being missed when it finishes after a newer job.
    ...
    const rows=db.prepare(`SELECT ${metadata} FROM ai_report_feed f JOIN ai_analysis_jobs j ON j.id=f.job_id WHERE f.cursor>? AND j.status='completed' ORDER BY f.cursor LIMIT ?`).all(cursor,size+1);
    const hasMore=rows.length>size,reports=rows.slice(0,size),latestCursor=db.prepare('SELECT COALESCE(max(cursor),0) AS n FROM ai_report_feed').get().n;
  ...
  function report(secret,id) { // Export only the finished document and descriptive metadata; prompt drafts, source snapshots and job controls stay in the console.
    ...
    const row=db.prepare(`SELECT ${metadata},j.document FROM ai_report_feed f JOIN ai_analysis_jobs j ON j.id=f.job_id WHERE j.id=? AND j.status='completed'`).get(id);
    if(!row)throw new ApiError(404,'Completed report not found.');return {...row,format:'markdown',incomplete:row.finish_reason==='length'};
  }
