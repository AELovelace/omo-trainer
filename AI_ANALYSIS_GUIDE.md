...
read-only token for MommyBot to retrieve completed nightly and explicitly shared documents through
[AI_REPORT_API.md](AI_REPORT_API.md). Reports are not published to the app.

Ordinary manual **Run now** reports stay in the admin console. **Run and share
with MommyBot** opts an on-demand report into the polling feed after completion.
The one scheduled job per Los Angeles midnight is also shared automatically;
delayed completion and retries retain the same report identity.
