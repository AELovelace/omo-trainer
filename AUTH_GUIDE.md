...
**Revocation:** Little Log Admin console → User management → Revoke sessions immediately signs an account out on every Little Log device. Disabling or changing its Little Log access also revokes sessions. Disabling an identity-service account or resetting its password removes identity-service sessions, but does not automatically revoke already-issued application sessions. For an immediate Little Log lockout, also revoke sessions or disable access in Little Log; otherwise its existing device session can last until the idle/absolute limits above. The identity service and each application retain separate revocation boundaries.
